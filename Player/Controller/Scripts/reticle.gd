...
#Essa função desenha o retículo
func _draw():
...
#Ajusta a posição do retículo
	RETICLE_LINES[0].position = lerp(RETICLE_LINES[0].position, pos + Vector2(0, -speed * RETICLE_DISTANCE), RETICLE_SPEED) #Top
	RETICLE_LINES[1].position = lerp(RETICLE_LINES[1].position, pos + Vector2(0, speed * RETICLE_DISTANCE), RETICLE_SPEED) #Right
	RETICLE_LINES[2].position = lerp(RETICLE_LINES[2].position, pos + Vector2(speed * RETICLE_DISTANCE, 0), RETICLE_SPEED) #Bottom
